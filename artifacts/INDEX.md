...
| **Color Correction Handbook** — Alexis Van Hurkman | Post — finishing: color grading (primary/secondary correction, hue curves, memory colors, shot matching, broadcast-safe QC) | **Imported · abridged** (10 ch, `0020`) into `reference/` + source INDEX. Grounds a future **Colorist** role + a *transform*-flavor **grade renderer**; scoped to grading only *(production-design concepts stay separate)* | [`color correction handbook/`](color%20correction%20handbook/INDEX.md) |
| **Directing the Story** — Francis Glebas | Development / direction — **visual storytelling**: the story → image bridge (story spine, character objectives, clarity of staging, sequence/beat construction) | **Imported · abridged** (10 ch — the *storytelling* half, `0043`) into `reference/` + source INDEX. Supplements the **Director** + **Screenwriter**; **no new `crew/` module**. *(Folder `directing the story/` ≠ Rabiger's `directing/`.)* | [`directing the story/`](directing%20the%20story/INDEX.md) |
| **The Art Direction Handbook** — Rizzo, 2nd ed. | Art department / production design — sets, palette *concepts*, the script → visual-concept design process | **Imported · abridged** (8 ch, `0044`→`0045`): verbatim `source/` + source INDEX (chapter → seat map), then all eight chapters transformed into session-ready `reference/` chapters, each closing in a **Studio application** section. Seats the **Production Designer** over `ImageStudio` — the dedicated source the `0041` sourcing note called for | [`the art direction handbook for tv and film/`](the%20art%20direction%20handbook%20for%20tv%20and%20film/INDEX.md) |
